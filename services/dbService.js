...
authenticateUrl="http://localhost:3000/api/authenticate";
...
service.getCustomerPassword = function(phone,callback){
...
service.getCustomerDetails = function(email,password,callback){
  var sql = "SELECT email,password FROM customer where email='"+email+"' and password='"+password+"' ";
  pool.getConnection(function(err, connection) {
    if(err) { console.log(err); callback([{}]); return; }
    // make the query
    connection.query(
